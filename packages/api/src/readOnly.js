...
export async function getAddressFromUID(uid) {
    ...
    const res = await callReadOnlyFunc(body, functionName)
    console.log(res.result)
    const address = cvToString(hexToCV(res.result)).replace('(some ', '').replace(')', '')
    console.log(address)
    return address
}

async function callReadOnlyFunc(body, functionName) {
    const req = await fetch(`${baseURL}/v2/contracts/call-read/${contractAddress}/${contractName}/${functionName}`, {
        method: "POST",
        headers: {
