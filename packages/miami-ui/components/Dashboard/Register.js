...
const Register = () => {
  ...
  useEffect(() => {
    getRegisteredMinersThreshold().then((result) => {
      setMinerThreshold(result);
    });
    getRegisteredMinerCount().then((result) => {
  ...
  function MinerButton() {
    ...
    if (minerCount == null) {
      ...
      return (
        <button className={styles.minersButtonActivated}>activated</button>
      );
    } else {
      return (
        <div className={styles.registerMiner}>
          <div className={styles.progress}>
            <div
              className={styles.progressBar}
              style={{ width: `${(minerCount / minerThreshold) * 100}%` }}
            >
          ...
          <button
            ...
            className={styles.registerToMineButton}
          >
            Register to mine
          </button>
        </div>
  ...
  return (
    <div className={styles.register}>
      ...
      <p>
        Before mining can begin, at least {minerThreshold} miners must register
